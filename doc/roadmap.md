...
### 2.2. Kotlinパーサーの実装 (解決済み)

- Kotlinコードを解析するための新しいパーサーモジュール（例: Kotlin Compiler APIや外部ツールを利用）を開発し、`CodeChunk` 形式に変換する。（解決済み）
- `AnalysisService` が言語タイプに基づいて適切なパーサーを選択できるようにする。（解決済み）
