...
- **`page`**: `number` (オプション)
  - 取得するページの番号（1から始まる）。デフォルトは1。
- **`pageSize`**: `number` (オプション)
  - 1ページあたりのアイテム数。デフォルトは適切な値（例: 50）。
...
- **`content`**: `Array<object>`
  - 処理結果のコンテンツ。以下の形式のオブジェクトを一つ含みます。
    - **`type`**: `string` (`"text"`)
    - **`text`**: `string`
      - パターンに一致するファイルの絶対パスのリストとページング情報をJSON文字列化したもの。以下のプロパティを持ちます。
        - **`items`**: `Array<string>`
          - 現在のページに含まれるファイルの絶対パスのリスト。
        - **`totalItems`**: `number`
          - 全ファイルの総数。
        - **`totalPages`**: `number`
          - 総ページ数。
...
- **`content`**: `Array<object>`
  - 処理結果のコンテンツ。以下の形式のオブジェクトを一つ含みます。
    - **`type`**: `string` (`"text"`)
    - **`text`**: `string`
      - ファイル内で見つかった関数のリストとページング情報をJSON文字列化したもの。以下のプロパティを持ちます。
        - **`items`**: `Array<object>`
          - 現在のページに含まれる関数のリスト。各オブジェクトは以下のプロパティを持ちます。
            - **`id`**: `string`
              - 関数のユニークなチャンクID。
            - **`signature`**: `string`
...
- **`language`**: `string` (オプション)
  - 解析対象ファイルの言語（例: `swift`, `kotlin`）。指定がない場合、ファイル拡張子から自動的に判断されます。
- **`pageSize`**: `number` (オプション)
  - 1ページあたりの行数。デフォルトはサーバー側で定義された閾値（例: 50）。
- **`pageToken`**: `string` (オプション)
  - 次のページまたは前のページをリクエストするためのトークン。
...
- **`content`**: `Array<object>`
  - 処理結果のコンテンツ。以下の形式のオブジェクトを一つ含みます。
    - **`type`**: `string` (`"text"`)
    - **`text`**: `string`
      - 関数のコード内容。ページングが適用された場合、以下のプロパティを含むJSON文字列。
        - **`content`**: `string`
          - 現在のページのコード内容。
        - **`isPartial`**: `boolean`
          - このチャンクが完全なコードチャンクの一部である場合に `true`。
        - **`totalLines`**: `number`
          - 元の巨大関数の総行数。
        - **`currentPage`**: `number`
          - 現在のページ番号。
        ...
        - **`nextPageToken`**: `string` (オプション)
          - 次のページをリクエストするためのトークン。
        - **`prevPageToken`**: `string` (オプション)
          - 前のページをリクエストするためのトークン。
...
- **`content`**: `Array<object>`
  - 処理結果のコンテンツ。以下の形式のオブジェクトを一つ含みます。
    - **`type`**: `string` (`"text"`)
    - **`text`**: `string`
      - 取得されたコードチャンクの内容。ページングが適用された場合、以下のプロパティを含むJSON文字列。
        - **`content`**: `string`
          - 現在のページのコード内容。
        - **`isPartial`**: `boolean`
          - このチャンクが完全なコードチャンクの一部である場合に `true`。
