...
export interface CodeChunk {
    ...
    type: 'function' | 'method' | 'class' | 'struct' | 'enum' | 'protocol' | 'var' | 'let'; // 型の正確な判別
    content: string; // コードの内容
    filePath: string; // 元のファイルパス
    startLine: number; // 開始行番号
    endLine: number; // 終了行番号
    byteOffset: number; // ファイル内でのバイトオフセット（SourceKittenから取得）
    byteLength: number; // コードのバイト長（SourceKittenから取得）
