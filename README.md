...
- **チャンク永続化層の分離**: `AnalysisService` からチャンクの永続化ロジックを独立したモジュールとして分離します。
...
- **パフォーマンス最適化（コードチャンクキャッシュ戦略の導入）**: `analyze_project` が呼び出されなくてもキャッシュが利用され、ファイルの更新やパーサーの仕様変更に追従できるキャッシュ戦略を実装します。
