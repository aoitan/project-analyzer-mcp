...
- `kotlin-parser-cli` のビルド（`test:integration` 内で自動実行）と、統合テストでの利用。
- `scripts/install_sourcekitten.sh` の CI 向け微修正（非対話、高速化、jq利用）。
...
1.  [ ] **Step 1: CI ワークフローの更新**
    - _Action_: `.github/workflows/ci.yml` に `full-integration-test` ジョブを追加。
    - _Action_: `swift-actions/setup-swift` または `actions/setup-swift` を使用して Swift 環境を構築。
    - _Action_: `scripts/install_sourcekitten.sh` を呼び出し、`sourcekitten` をインストール（GITHUB_TOKEN 利用）。
2.  [ ] **Step 2: 統合テストの実行**
    - _Action_: `npm run test:integration` を実行し、Swift/Kotlin 両方の解析が CI 上で動作することを確認。
3.  [ ] **Step 3: ガードレールの確認**
    - _Validation_: `sourcekitten` が見つからない場合にテストが適切に失敗（またはスキップ）されているか確認。
